...
| GET | `/version` | — | Канон версии для 1C77 KKT Updater (`component` + `version`) |
| POST/GET | `/exit` | — | Штатное завершение процесса (для updater; закрытие окна — в трей) |
...
Контракт **1C77 KKT Updater** (CP3). Без опроса ККТ.

**Ответ:**

```json
{
  "component": "atol-service",
  "version": "2026.08.11-01"
}
```

`version` — канон `YYYY.MM.DD-NN` (из ProductVersion или нормализованного FileVersion).

---

## 4.2. `POST /exit` / `GET /exit`

Штатное завершение процесса ККТ-сервера. Нужно **1C77 KKT Updater** и админ-сценариям: закрытие окна пользователем **не** завершает процесс — окно уходит в системный трей (двойной клик по иконке — восстановить).
...
После ответа процесс завершается. Если HTTP-сервер остановлен кнопкой «Стоп», endpoint недоступен.
